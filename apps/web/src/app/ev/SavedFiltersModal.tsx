...
import { deleteSavedEvFilter, type EvFilterState, type SavedEvFilter } from "../../lib/evFilters";

type Props = {
  open: boolean;
  onClose: () => void;
  savedFilters: SavedEvFilter[];
  onApply: (filters: EvFilterState) => void;
  onDeleted: (next: SavedEvFilter[]) => void;
};

export function SavedFiltersModal({ open, onClose, savedFilters, onApply, onDeleted }: Props) {
  return (
    <Modal open={open} onClose={onClose} widthClassName="max-w-sm">
      <div className="flex flex-none items-center justify-between gap-3 border-b border-border p-4">
        <div className="text-[15px] font-bold">Filtros salvos</div>
          ...
          >
            ...
            >
              ...
              <div className="font-mono text-[10px] text-text-tertiary">
                {f.filters.dateFrom} → {f.filters.dateTo}
              </div>
            </button>
            <button
              onClick={() => onDeleted(deleteSavedEvFilter(f.id))}
